...
WireTextEditor.Modes.Default = {
	-- TODO the default mode should probably hold all language-neutral colors (eg. dblclickhighlight)
	SyntaxColors = { text = Color(255, 255, 255, 255) },
	SyntaxColorLine = function(self, row)
		return { { self.Rows[row], { WireTextEditor.Modes.Default.SyntaxColors["text"], false } } }
	end,
}
...
function EDITOR:SetMode(mode)
	mode = mode or WireTextEditor.Modes.Default
	self.CurrentMode = mode
	-- do we need to manually refresh here to update syntax coloring?
end
...
function EDITOR:SyntaxColorLine(row)
	self.CurrentMode:SyntaxColorLine(row)
end
...
function EDITOR:BlockCommentSelection( removecomment )
	...
	self.CurrentMode.BlockCommentSelection(self, removecomment)
